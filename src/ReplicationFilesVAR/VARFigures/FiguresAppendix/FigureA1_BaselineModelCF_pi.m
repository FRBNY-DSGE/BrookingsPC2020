...
y=[unem nunem corepceinfl gdpinfl 100*(rgdp-log(pop)) 100*(h-log(pop)) aheinfl ls pceinfl cpiinfl corecpiinfl];
series=["unemployment","natural unemployment","core inflation","inflation","GDP","hours","wage inflation (PNSE)","labor share","inflation (PCE)","inflation (CPI)","core inflation (CPI)"]
...
enddate2=2008.5; T12=find(Time==enddate2);
...
% plotg the impulse responses of unemployment and inflation
qqq=[.025 .16 .5 .84 .975];     % percentiles of the posterior distribution
indfig=[1 9 10 11];            % variable position of impulse responses to plot
...
subplot(2,2,1); plot([0:H]', medU,'--','LineWidth',3,'color',[.2157, .4941, .7216]);
