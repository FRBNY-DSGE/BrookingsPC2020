...
enddate1=1983.75; T11=find(Time==enddate1);

% second sample
startdate2=1984-lags/4; T02=find(Time==startdate2);
...
CF1(:,9,:)=4*(iw1-lag(iw1));        % implied response of annualized wage inflation (total economy)
sCF1 = sort(CF1,3);
...
CF2(:,9,:)=4*(iw2-lag(iw2));        % implied response of annualized wage inflation (total economy)
sCF2 = sort(CF2,3);

% plot the impulse responses of all variables
qqq=[.025 .16 .5 .84 .975];         % percentiles of the posterior distribution
indfig=[1 2 3 4 7 9 8 5 6];         % variable position of responses to plot
...
aux=subplot(3,3,1); plot([0:H]', medU,'--','LineWidth',3,'color',[.2157, .4941, .7216]); 
limitsy=get(aux,'ylim'); subplot(3,3,2); ylim(limitsy);
subplot(3,3,1); plots=get(gca, 'Children'); legend(plots([7 4]),{'pre 1984','post 1984'},'Location','northeast');