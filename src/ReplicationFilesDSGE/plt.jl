...
"""
```
plt_solve(m::AbstractDSGEModel)
```

Solves for the transition equation of `m` under a price level
targeting rule (implemented by adding a price-gap state)
"""
function plt_solve(m::AbstractDSGEModel)

    # Get equilibrium condition matrices
    Γ0, Γ1, C, Ψ, Π  = plt_eqcond(m)

    # Solve model
    TTT_gensys, CCC_gensys, RRR_gensys, eu = gensys(Γ0, Γ1, C, Ψ, Π, 1+1e-6)
@show size(TTT_gensys)
    if !((eu[1] == 1) & (eu[2] == 1))
...
"""
```
init_plt_forecast(m::AbstractDSGEModel, shocks::Matrix{T}, final_state::Vector{T})
```

Adjust shocks matrix and final state vector for forecasting under the PLT rule
"""
function plt_forecast_init(m::AbstractDSGEModel, shocks::Matrix{T}, final_state::Vector{T};
